...
void MosaicScenarioManager::initialize() {
    ...
    sched = dynamic_cast<MosaicEventScheduler*>(cSimulation::getActiveSimulation()->getScheduler());
    if (nullptr == sched) {
        EV_ERROR << "MosaicEventScheduler has to be used as a scheduler!" << std::endl;
    }
...
void MosaicScenarioManager::handleMessage(cMessage *msg) {
    ...
    EV << "handleMessage() - name: " << msg->getName() << ", kind: " << msg->getKind() << std::endl;
    ...
    if (strcmp(msg->getName(), "MosaicMobilityCmd") == 0) {
        ...
        if (cmd->getCmdType() == MOBILITY_CMD_ADD_NODES) {
            ...
            for (unsigned int i = 0; i < cmd->getNodeIdArraySize(); i++) {
                ...
                EV_DEBUG << "MosaicScenarioManager MOVE_NODES: " << nodeId
                        << " to position " << position.x << "," << position.y << std::endl;
                moveNode(nodeId, position);
            }
        } else if (cmd->getCmdType() == MOBILITY_CMD_REMOVE_NODES) {
            for (unsigned int i = 0; i < cmd->getNodeIdArraySize(); i++) {
                nodeId = cmd->getNodeId(i);
                EV_DEBUG << "MosaicScenarioManager REMOVE_NODES: " << nodeId << std::endl;
                removeNode(nodeId);
            }
        }
        ...
        // (mosaic tcp app is not activated up to now, as no real use case exists for simulation)
...
void MosaicScenarioManager::addModule(int nodeId, std::string type, std::string name, std::string displayString) {
    if (nodes.find(nodeId) != nodes.end()) {
...
void MosaicScenarioManager::addNode(int nodeId, inet::Coord& position) {
    ...
    EV << "MosaicScenarioManager added vehicle " << nodeId << " at position " << position.x << "," << position.y << " at time " << simTime() << std::endl;
}
...
void MosaicScenarioManager::addRsuNode(int nodeId, inet::Coord& position) {
    ...
    newNode->callInitialize();
    EV_DEBUG << "MosaicScenarioManager added rsu " << nodeId << " at position " << position.x << "," << position.y << " at time " << simTime() << std::endl;
}
...
void MosaicScenarioManager::moveNode(int nodeId, inet::Coord& position) {
    ...
    if (!mod) {
        ...
        EV_DEBUG << "MosaicScenarioManager moved vehicle " << nodeId << " to position " << position.x << "," << position.y << " at time " << simTime() << std::endl;
    }
}
...
void MosaicScenarioManager::removeNode(int nodeId) {
    ...
    if (!mod) {
        ...
        nodes.erase(nodeId);
        mod->callFinish();
        //we have to delete the two radios before we delete the node, since the MediumLimitCache tries
        //to determine the minimal constrained area from the nonexistent mobility while deleting the radios.
        ...
        EV << "MosaicScenarioManager removed node " << nodeId << " at time " << simTime() << std::endl;
    }
}
...
void MosaicScenarioManager::sendV2xMessage(cMessage *msg) {
    ...
    if (!mod) {
        ...
        if (cmd->getTtl() != 1) {
            EV << "WARNING: up to now only singlehop-broadcast supported" << std::endl;
        } else {
            ...
            EV << "MosaicScenarioManager send udp message " << msgId << " from node " << nodeId << " at time " << simTime() << std::endl;
        }
    }
}
...
void MosaicScenarioManager::receiveV2xMessage(cMessage *msg) {
    ...
    EV_DEBUG << "MosaicScenarioManager receive v2x message " << packet->getMsgId() << " on node " << packet->getNodeId() << " at time " << simTime() << " on channel " << packet->getChannelId() << std::endl;
